class Admin::AnswersController < Admin::AdminController
    ...
    def create
        ...
        respond_to do |format|
            if @answer.save
                ...
                format.html { redirect_to admin_answer_path(@answer), notice: @controller_name + t(:message_success_insert) }
                format.json { render json: @answer, status: :created, location: @related_action }
            else
    ...
    def answer_params
        params.require(:answer).permit(:question_id, :title, :enable)
    end
end
